...
public:
	...
	//所有socket上的事件被注册到同一个epoll内核事件表,所以将epoll文件描述符设置为静态
...
private:
	...
	//采用writev来执行写操作,所以定义下面两个成员,其中m_iv_count表示被写在内存块的数量
	struct iovec m_iv[2];
	int m_iv_count;
};
#endif
